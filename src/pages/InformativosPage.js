...
const InformativosPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchPosts = async () => {
            try {
                // 1. Encontrar o ID da categoria "Informativos" (slug=informativos)
                const categoryRes = await fetch(`${API_BASE_URL}/categories?slug=informativos`);
                const categories = await categoryRes.json();
                
                if (categories.length === 0) {
                    throw new Error('Categoria "Informativos" não encontrada na API.');
                }
                ...
                setError('Não foi possível carregar os informativos. Tente novamente mais tarde.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <PageContainer>
            <ContentHeader>
                ...
                <p style={{ color: '#666' }}>Documentos e comunicações oficiais do nosso escritório.</p>
            </ContentHeader>

            {loading && <LoadingMessage>Carregando informativos...</LoadingMessage>}
            {error && <LoadingMessage style={{color: 'red'}}>{error}</LoadingMessage>}
            
            {!loading && !error && posts.length === 0 && (
                <LoadingMessage>Nenhum informativo encontrado no momento.</LoadingMessage>
            )}
